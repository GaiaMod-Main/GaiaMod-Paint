...
const fetchFAIcons = () => {
    if (_faIconCache) return Promise.resolve(_faIconCache);
    if (_faFetchPromise) return _faFetchPromise;
    _faFetchPromise = fetch(FA_JSON_URL)
        .then(r => r.ok ? r.json() : null)
        .then(data => { _faIconCache = data; return data; })
...
class FontAwesomeSearchPanel extends React.Component {
    ...
    componentDidMount () {
        fetchFAIcons().then(data => {
            this.setState({ loading: false, results: searchFACache('', data, 2141) });
